...
Boundary Events can be attached to tasks (and task-like activities), to Sub-Processes and Call Activities.  This section covers behavior of boundary events when applied to sub-processes.  In general, boundary events on a Call Activity behave similarly to those on a Sub-Process, but see this section on the [Call Activities page for details].[callActivities.md]({{ site.url }}{{ site.baseurl }}/{{ page.collection }}/callActivities/###setting-boundary-events-on-call-activities)
