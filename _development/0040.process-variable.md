...
If your process instance uses bpmn:callActivities and calls models contained on a separate diagram, information on process variable scoping system and how to access a process variable by scope is included [on this page]({{ site.url }}{{ site.baseurl }}/{{ page.collection }}/callActivities/).. In addition, tasks or subProcesses that loop or iterate also use scoping to keep identically named process variables separate in each loop or iteration. 
...
The following pseudo variables are available in BPMN Loops and Iterations (in Flows for APEX Enterprise Edition).  

| Pseudo Variable | Description | Available |
|---|---|---|
| loop_counter | the loop or iteration counter (1-n) | Always Available |
...
### JSON typed Process Variables - New in v24.1 (all editions)
...
A JSON process variable can be `set` or `get` in either CLOB format (to get a character representation of the JSON) using `get_var_json`, or as a `json_element_t` typed JSON object using `get_var_json_element`.

### Persistence

Process variables are persistent for the duration of the flow instance, being stored inside the database until the process instance is deleted with a `flow_delete` call or a `flow_reset` (with the exception of built-in variables which are retained after a flow_reset).
