...
The Gateway Routing Expression can be either a PL/SQl expression, or a PL/SQL Function Body.  The Routing expression can contain references to process variables, using either substitution syntax `&F4A$myvar.` or bind syntax `:F4A$Myvar`.
...
2. Failing that, the Default Path is chosen.  See 'Default Paths on Inclusive Gateways' below.
3. If no routing is provided, and no default is specified, an error is signaled.
