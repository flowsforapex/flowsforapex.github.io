...
## Async Task Behavior After Rewind

When a process instance has been rewound and execution is resumed, the async execution behavior changes temporarily to facilitate debugging and controlled restart. This section explains how async tasks behave after a rewind operation.

### Foreground Execution After Resume

After a process instance has been rewound and you issue the **Resume** command, tasks that would normally run in an async background session will instead **run in the foreground session** of the user issuing the Resume command.

This foreground execution behavior applies to:
- Tasks with **Async Before** or **Async After** flags set
- Tasks triggered by **Timer Events** (which normally run via DBMS_Scheduler)
- Tasks processing **incoming messages via AQ** (which normally run in async sessions)
- Any other tasks that would normally execute asynchronously

The foreground execution continues until either:
- Another explicit **Async Before** or **Async After** flag is encountered on a subsequent task, OR
- A **natural break point** is reached (User Task, Message Catch Event, Timer Catch Event, etc.)

### Why This Happens

This temporary foreground execution behavior after rewind ensures that:

1. **Immediate Feedback** - Administrators can observe execution results immediately after resume without waiting for background jobs
2. **Easier Debugging** - Synchronous, visible execution makes it easier to verify that the rewind corrected the issue
3. **Controlled Restart** - Any corrections or adjustments take effect immediately in the current session
4. **No Unexpected Background Jobs** - Prevents inadvertently triggering background jobs during recovery operations

### Example Scenario

Consider a process where:
1. A Timer Event fires (normally creates DBMS_Scheduler job - async)
2. Followed by a Script Task with **Async Before** flag (normally async)
3. Followed by several more Script Tasks
4. Ending with a User Task (natural break point)

**Normal execution:**
- Timer fires → DBMS_Scheduler creates background session
- Background session processes all tasks until User Task
- Execution is fully async

**After Rewind and Resume:**
- Administrator issues Resume command
- Timer-triggered step executes in administrator's session (foreground)
- Script Task with Async Before executes in administrator's session (foreground)
- All subsequent Script Tasks execute in administrator's session (foreground)
- Execution stops at User Task
- administrator sees immediate results of all steps

### Resuming Normal Async Behavior

Once the rewound instance encounters one of the following, normal async behavior resumes for subsequent processing:

1. **A new task with an Async Before or Async After flag** - The flag triggers async execution as configured
2. **A Timer Event fires** - Creates a DBMS_Scheduler session as normal
3. **A message arrives via AQ** - Creates an async session for message processing as normal

From that point forward, async execution follows the normal rules until the process completes or encounters another issue requiring intervention.

### Best Practices When Rewinding Async Processes

1. **Expect Synchronous Execution** - After resume, be prepared for immediate execution of steps that normally run async
2. **Monitor the First Resume** - Watch the Instance Timeline carefully during the first resume after rewind to verify correct behavior
3. **Test in Development** - Practice rewind/resume operations in development or test environments before using in production
4. **Check Background Sessions** - After normal async behavior resumes, verify that background jobs are running as expected
5. **Use Instance Timeline** - Review the Instance Timeline to understand which steps ran synchronously vs. asynchronously
6. **Consider Transaction Boundaries** - Remember that foreground execution may have different transaction characteristics than async execution

### Visibility in Instance Timeline

The Instance Timeline view shows:
- Which steps executed in foreground session after resume
- When normal async behavior resumed
- Background job execution details for subsequent async steps
- Clear indication of the transition from post-rewind foreground execution to normal async execution

This visibility helps administrators understand exactly how the process executed after rewind and verify that it's behaving as expected.

For more information about async execution and workflow sessions, see [Async Tasks and Background Execution](/{{page.collection}}/async-tasks-and-background-execution/).
