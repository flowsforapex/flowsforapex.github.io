---
permalink: /:collection/starter-workshop/
title: "Build your First Workflow Workshop"
toc: true
typora-root-url: ../
---
...
**Duration**: This workshop takes about 60-90 minutes to complete, but afterwards you should be able to model a business process and turn that into a Flows for APEX application.
...
2.  To save time, we have created a basic application (**Flows for APEX Starter Workshop**) that you will enable for Flows for APEX.  You should now download the [workshop starter APEX app](/assets/files/FlowsForApexStarterWorkshopStarter251APEX242.sql).  **Install it** into your APEX workspace, **including the supporting objects**. This installs:
	...
	- `Approve Expenses` APEX Human Task definition (we will enhance this during the workshop)
...
- Select the Gateway, click on the Task symbol ( <span class="bpmn-icon bpmn-icon-task-none"></span> ), labeled **Append task**.  Name that **Request Payment**.  Then select the gateway again, and append another task.  Drag this To above Manager Approval, and rename it **Correct Expense Report**.
...
- Now add another task after Request Payment, and rename it **Send Email**.  Then append an **End Event** ( <span class="bpmn-icon bpmn-icon-end-event-none"></span>).  Name the End Event with the resulting business state, "**Expenses Paid**".
...
- Using the **+ Create Instance** button at the top right, **Create a Flow Instance** of our **Expense Report** model. 
  ...
  - Logging Level: You can leave blank now, but in future this gives you the ability to set a higher logging level for a manually started process run - useful for debugging.
...
![Test Instance in Flow Monitor - un-started](/assets/images/wksp1-test-instance-created.png)
...
- In the Flow Monitor, we can **set gateway routing** and then **restart an errored process step**.  Open the **Variables** tab, select **Set Gateway Routing**.  In the modal dialog, select the **Calling Diagram** (Main Diagram), the **Gateway**, (Approved?), and set it to go on the **approved** path.  **Add the Process Variable**.  Then select the **Subflows** tab, and **re-start** the subflow.
...
You now have all of the plugins that you need.  We've also created three **application items** that will be used to hold the **process_ID**, **subflow_ID**, and **step_key** when a step starts.  These three parameters always need to be returned when a step completes.
...
For this workshop, we have done this for you - the Task List page in the **Flows for APEX Starter Workshop** app already contains a configured Task List page.  But so you know how to set this up on your next application project, all you need to do is follow the instructions in our documentation [how-to page on the APEX Task List](/{{page.collection}}/apex-task-list/).
...
- Select the **Manager Approval** task.  Using the spanner / wrench tool ( <span class="bpmn-icon bpmn-icon-screw-wrench"></span> ), convert our generic task type to a **User task**.
...
- Open the **APEX Human Task** region:
  ...
  - Business Reference:  Click on the blue link named **business ref** underneath the item.  This will load the value `&F4A$BUSINESS_REF.` into the item.  (Note:  Flows for APEX process variables can be substituted using APEX `&name.` syntax -- but we always prefix the variable name with (upper case) `F4A$` to prevent APEX item names and Flows for APEX variable names getting confused.)
  - Parameters.  The APEX Human Task requires some task parameters to be set.  If you click on the green **Load Parameters** link, we will pre-populate a parameter for each expected parameter.   And we'll set the value for `process_id`, `subflow_id`, and `step_key` for you as well.
...
- Now we're going to configure the Gateway routing to use the Outcome from our Approval Task.  Select not the gateway but the link (Sequence Flow)  between the **Approved?** gateway and the **Request Payment Task** - the **approved sequence flow**.  In the properties panel, open the **condition** region, then:
  - Sequence: **10**
...
- Create the expense report.  Our expense report was created.
...
​	That's **not** what we were expecting.  Let's look at the process variables in the **Variables** tab.  You can see that the `BUSINESS_REF` is set, but our `APPROVAL_OUTCOME` doesn't exist.  We didn't do anything in our APEX Human Task to return the outcome to Flows for APEX.  Let's go and fix that...

- Open our **Flows for APEX - Starter Workshop** app in the APEX app builder.  Go to **Shared Components** and then to **Task Definitions**.  Open the **Approve Expenses** approval task.
  Inspect the task definition.  Note that:
  - We set a task subject using some substitution parameters.  All good.
  ...
  - Actions.  There aren't any **task actions** defined!  So our approval task doesn't yet do anything when it completes.  Here's our problem.  We need to add a **task action** for **task completion**, and we should also add actions for **cancellation**, **expiry** (if our Human Task has expiration rules - which it doesn't), and (after APEX 26.1 arrives) **error**.
- Click on **Add Action**
...
- Click on **Add Action**
  ...
  - On Event: **Cancel**
  - Note the Action Settings have automatically appeared, naming the **task parameters** containing **process_id**, **subflow_id**, and **step_key**.  All good,
- If you are using APEX 26.1+, you'll then do the same for **expiration** (if you have defined task expiration), and **error** (this doesn't exist yet!).
- Click **Apply Changes**
...
Let's go back to the **Flow Manager** in the **Flows for APEX app**, and complete the details of what happens to an **approved** expense report.  
...
In **Flow Management**, find our **Expense Report** diagram, and open it in the **Flow Modeler** again.

- Select the **Request Payment** task.  Use the spanner / wrench tool, change this to a **Script Task** (  <span class="bpmn-icon bpmn-icon-script-task"></span> ).
...
- The **Binding** options below the code editor are **important**.  
  If your script code contains references to Flows for APEX process variables, select **Allow Binding** and then select **Bind Process Variables**.
...
- Flows for APEX supports several **Service Task Types**.  Select **Send Mail**.
- Open the **Mail** region, and configure as below:
  ...
  - To: For the demo, hard code this to your email address.  (The actual demo scenario would set this to `&F4A$employee_username.` - but as this email address likely isn't valid, you will get an error from the mail task! )
  - Use Template:  **on**
  ...
  - Placeholder:  Click the **Load JSON** green button & then open the code editor.  You will see a JSON document template listing the placeholders required for the selected email template.
...
- We will use variable expressions on the **Send Email** task that execute **before task** to set the email addressee, the category, amount, and expense date (as a string) from our system of record for use in our email template.
  ...
  - Open the **Variable Expressions** region on our **Send Email** task.  Click '**+**' to add a variable expression.
  - Sequence:0
  - Name: EMAIL
  - Data Type: Varchar2
...
We can test our **approved** route through the model now by creating another expense report and approving it.
...
## Download a Completed Solution

If you want to download a completed application, [download here](/assets/files/FlowsForAPEXStarterWorkshopComplete251APEX242.sql).  Note that the completed application uses a BPMN diagram named **Completed Expense Report**, rather than **Expense Report** so that it doesn't conflict with the starter app.  
When you install the completed application and then install the **Supporting Objects**, you will get some errors when supporting objects are installed if you have already installed the starter app.  This will be the 2 underlying tables - just ignore this error.  The supporting object install should also install the completed process diagram for you.

## If you have a Problem - Debugging an Error

If something is not working for you, you'll want to turn on some logging or debugging in the workflow.  You can do this by setting the **logging level** for all instances of your process diagram to the highest level.  You can do this at **diagram level**, or at **system level**, capturing maximum logging for all diagrams.

### Enable Logging Level at Diagram Level

- Open our **Expense Report** diagram in the **Flow Modeler**.
- Select the **Process** by clicking on the diagram canvas, rather than any of the objects in the diagram.
- Open the **Execution** section in the **Properties Panel**.
- Set the **Logging** level to **Full (8)** for the diagram.  This setting allows you to specify the minimum logging level at which all instances of this process diagram will be logged at.  (In production use, you might set the logging level for your security-relevant payment process to capture all events (Full/Debug), while leaving your less security-relevant 'Apply for a Parking Space' process to only capture Abnormal Events (level 1).).

## Summary


In this workshop, we've modeled and built a small process that uses several of the main task types, and you have seen how to integrate these into an APEX application.  This is a very simple process - but you should now be able to go out and model and implement much bigger business processes.  Happy Automating!
