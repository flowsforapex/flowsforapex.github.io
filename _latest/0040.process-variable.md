...
**Loops and Iterations.**  🆕(Enterprise Edition ony).  Tasks or subProcesses that loop or iterate also use scoping to keep identically named process variables separate in each loop or iteration.
...
Process variables persistfor the duration of the flow instance, being stored inside the database until the process instance is deleted with a `flow_delete` call or a `flow_reset` (with the exception of built-in variables which are retained after a flow_reset).
