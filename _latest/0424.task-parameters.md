...
In Flows for APEX v26.1, Task Parameters are implemented for:
- All PL/SQL type tasks (Script Tasks, Service Tasks (execute PL/SQL type), Business Rule Tasks (execute PL/SQL type), Send Tasks (execute PL/SQL type))
- For all tasks inside a Adhoc SubProcess.

Implementation of parameters are planned to follow in other task and event types in future releases.
...
Any parameters that are specifed as `userInput` for Adhoc SubProcesses are collected by an automatically generated APEX form.  The form uses the `ORACLE-APEX-JSON-REGION` region plugin from Uwe Simon to dynamically generate an APEX region at runtime containing the required parameters.
