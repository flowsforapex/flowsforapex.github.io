...
Flows for APEX has a transactional model which ensures that each step in the model is completed as a transaction, allowing step rollback and restart in the event of errors.  Developers should be aware of the Flows for APEX Transaction Model before designing applications using Flows for APEX.
...
![Example Process Model](/assets/images/ShipmentProcess.png){: .align-center}

In a typical implementation of a business process, each step in the Process Model would be implemented as a database transaction.  Sometimes a step might do several database transactions, but then the step would be responsible for managing the coordination of transactions within a step.  But a  transaction shouldn't normally span multiple process steps.
...
![Process Step Phases](/assets/images/transaction-step-phases2.png){: .align-center}
...
**Note:** In Flows for APEX Enterprise Edition, tasks can be configured to execute asynchronously using Async Before and Async After flags. When these flags are set, the Pre-Task or Post-Task Phase commits occur before background execution begins. For more information about async execution and workflow sessions, see [Async Tasks and Background Execution](/{{page.collection}}/async-tasks-and-background-execution/).
...
![Example Vacation Request Model](/assets/images/transactions-example1.png){: .align-center}
...
![Example Vacation Request Model](/assets/images/transactions-example-1a.png){: .align-center}
...
![Example Vacation Request Model](/assets/images/transactions-example2.png){: .align-center}
...
![Example Vacation Request Model](/assets/images/transactions-example3.png){: .align-center}
...
![Showing Errors In the Current Step](/assets/images/transactions-error1.png "Showing Errors In the Current Step"){: .align-center}
...
![Showing Errors In the Next Step](/assets/images/transactions-error2.png "Showing Errors In the Next Step"){: .align-center}
...
- Any errors that occur when a Subsequent Step is being processed will not return an error message to the user performing the Current Step.  Instead:
  ...
  - the Flow Instance containing the error will be put into `error` status.
    ...
    ![subflowWithError](/assets/images/subflowInError.png "Showing Errors on a Subflow"){: .align-center}
...
![Setting Instance Operations inside APEX Page Processing Correctly](/assets/images/transactions-apex-designer1.png "Setting Instance Operations inside APEX Page Processing Correctly"){: .align-center}

![Setting Instance Operations inside APEX Page Processing](/assets/images/transactions-apex-designer2.png "Setting Instance Operations inside APEX Page Processing Badly"){: .align-center}
...
- [Async Tasks and Background Execution](/{{page.collection}}/async-tasks-and-background-execution/) - Understanding workflow sessions and async execution
- [PL/SQL Script Tasks](/{{page.collection}}/script-task/) - Configuring Script Tasks with Async Before/After flags
- [Process Variables](/{{page.collection}}/process-variable/) - Using Process Variables across transaction boundaries
