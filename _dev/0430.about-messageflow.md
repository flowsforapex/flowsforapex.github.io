...
**Async Execution:** Incoming messages received via AQ are processed in an **async background session**, separate from any user session. The workflow engine processes the message and continues executing steps until reaching a natural break point. For more information about async execution and workflow sessions, see [Async Tasks and Background Execution](/{{page.collection}}/async-tasks-and-background-execution/).
