...
Starting from Flows for APEX v26.1, JSON-typed process variables can also be read declaratively from Variable Expressions using the `jsonPath` expression type.  This provides a simple way to extract scalar values or JSON fragments from a JSON process variable without having to write PL/SQL.

See [Variable Expressions](/{{page.collection}}/variable-expression/) for details.
