...
The adhoc sub process continues until its completion condition is met.

## The Manual Adhoc Sub Process Pattern

In a manual adhoc sub process:
...
The normal manual adhoc sub process flow looks like this:

1. the adhoc sub process becomes active
...
8. if the adhoc sub process is not finished, the next set of currently startable activities is shown

This cycle repeats until the completion condition becomes true.

## Designing for Knowledge Workers

Manual adhoc sub processes work best when users can quickly understand:
...
Good manual adhoc sub process design includes:
...
Manual adhoc sub process activities often need user-provided input.

Task Parameters are therefore central to manual adhoc sub process design.
...
In practical terms, this means the adhoc sub process should present:
...
## Completion in Manual adhoc sub processs

Even in a manual adhoc sub process, completion should not depend on the user guessing when to stop.
...
For manual adhoc sub processs, good practice is to:
...
## Relationship to AI-Driven Adhoc Sub Processs

Manual and AI-driven adhoc sub processs share the same base model.

If your adhoc sub process is designed well for manual use, it is usually much easier to evolve it later into an AI-assisted or autonomous pattern.
